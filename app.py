...
# ------------------------------------------------------------
# 2. Funciones de análisis (iguales que antes, pero con geotermometría opcional)
# ------------------------------------------------------------
def convertir_a_mg_L(valor, unidad, parametro):
    """Convierte a mg/L según la unidad seleccionada"""
...
def balance_ionico(datos_mg):
    ...
    meq = {}
    for ion, valor in datos_mg.items():
        if ion in pesos and valor and valor > 0:
            meq[ion] = (valor * valencias[ion]) / pesos[ion]
        else:
            meq[ion] = 0
    ...
    if abs(error) < 5:
        ...
        if cationes > aniones:
            diag += " - Exceso de cationes"
        else:
            diag += " - Exceso de aniones"
    return meq, cationes, aniones, error, diag

def kurlov(meq):
    suma_cat = meq['Ca']+meq['Mg']+meq['Na']+meq['K']
    suma_an = meq['HCO3']+meq['SO4']+meq['Cl']
    porc_cat = {ion: meq[ion]/suma_cat*100 for ion in ['Ca','Mg','Na','K']} if suma_cat > 0 else {}
    porc_an = {ion: meq[ion]/suma_an*100 for ion in ['HCO3','SO4','Cl']} if suma_an > 0 else {}
    cationes = [ion for ion in ['Na','Ca','Mg','K'] if porc_cat.get(ion,0) >= 20]
    aniones = [ion for ion in ['HCO3','SO4','Cl'] if porc_an.get(ion,0) >= 20]
    simb_an = {'HCO3':'HCO3', 'SO4':'SO4', 'Cl':'Cl'}
    simb_cat = {'Na':'Na', 'Ca':'Ca', 'Mg':'Mg', 'K':'K'}
    return " · ".join([simb_an[a] for a in aniones]) + " – " + " · ".join([simb_cat[c] for c in cationes])
...
# Mostrar contador de muestras usadas
...
def descargar_plantilla():
    # Crear un DataFrame con una fila de ejemplo
    ...
    href = f'<a href="data:file/csv;base64,{b64}" download="plantilla_modelagua.csv">📥 Descargar plantilla de ejemplo (.csv)</a>'
    return href

st.markdown(descargar_plantilla(), unsafe_allow_html=True)

# ------------------------------------------------------------
# 3.2 Sección: Guía de formato (desplegable)
# ------------------------------------------------------------
